...
// MARK: - Protocol (test edilebilirlik için)
...
final class CameraService: NSObject, CameraServiceProtocol, ObservableObject {
    ...
    // Frame'leri dışarı iletmek için (ViewModel dinleyecek)
    let framePublisher = PassthroughSubject<CMSampleBuffer, Never>()
    
    // MARK: - Private
    private let session = AVCaptureSession()
    private let videoOutput = AVCaptureVideoDataOutput()
    private let sessionQueue = DispatchQueue(label: "com.fitcounter.camera.session")
    private let outputQueue = DispatchQueue(label: "com.fitcounter.camera.output")
...
extension CameraService: AVCaptureVideoDataOutputSampleBufferDelegate {
    func captureOutput(
        ...
        // Her frame ViewModel'e iletilir
